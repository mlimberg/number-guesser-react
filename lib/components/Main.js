...
export default class Main extends Component {
  ...
  handleRange(input) {
    if(input) {
      this.setState({ min: +input.min, max: +input.max, error: false, guess: null, }, ()=> { this.setNewRandomNumber() })
    } else {
  ...
  render() {
    return(
      <div id='appContainer'>
        ...
        {this.state.error ? this.displayError() : console.log('hey')}
        ...
        <div id='rangeSection'>
          ...
          <button className='button'
            id='reset-game-btn'
            // disabled={!this.state.guess || !this.state.error}
